...
import { Calendar, ExternalLink, Mail, Share2, Copy, Twitter, Linkedin } from "lucide-react";
import { useState } from "react";
import { toast } from "sonner";

interface DigestItem {
  date: string;
  category: "FCA" | "AML/KYC" | "MiFID II" | "Crypto" | "SMCR" | "ESG" | "Consumer Duty";
  title: string;
  summary: string;
    ...
    items: [
      {
        ...
        impact: "Medium",
        sourceUrl: "https://www.fca.org.uk/publications/policy-statements/ps26-6-senior-managers-certification-regime-review",
      ...
      },
      {
        ...
        impact: "Medium",
        sourceUrl: "https://www.fca.org.uk/publications/good-and-poor-practice/consumer-understanding-good-practice-areas-improvement",
      ...
      },
      {
        ...
        impact: "High",
        sourceUrl: "https://www.fca.org.uk/publications/policy-statements/ps26-3-motor-finance-consumer-redress-scheme",
  ...
  },
  {
    ...
    items: [
      ...
      },
      {
        ...
        impact: "Medium",
        sourceUrl: "https://www.fatf-gafi.org/en/publications/High-risk-and-other-monitored-jurisdictions/increased-monitoring-october-2025.html",
...
const categoryColours: Record<string, string> = {
  ...
  "MiFID II": "bg-emerald-100 text-emerald-800 dark:bg-emerald-900/30 dark:text-emerald-300",
  Crypto: "bg-violet-100 text-violet-800 dark:bg-violet-900/30 dark:text-violet-300",
  ...
  "Consumer Duty": "bg-orange-100 text-orange-800 dark:bg-orange-900/30 dark:text-orange-300",
};
    ...
    <PageLayout>
      ...
      <section className="max-w-4xl mx-auto px-4 py-12">
          ...
          {DIGESTS.map((digest) => (
            <div key={`${digest.month}-${digest.year}`}>
              ...
              <div className="flex items-center gap-3 mb-6">
                ...
                <span className="text-xs text-muted-foreground">{digest.items.length} update{digest.items.length !== 1 ? "s" : ""}</span>
              </div>
                ...
                {digest.items.map((item, idx) => (
                  ...
                  >
                    <div className="flex flex-wrap items-center gap-2 mb-3">
                      <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${categoryColours[item.category] ?? ""}`}>
                        {item.category}
                      </span>
                      <span className={`text-xs font-semibold px-2.5 py-0.5 rounded-full ${impactColours[item.impact]}`}>
                        {item.impact} Impact
                      </span>
        ...
        <div className="mt-12 p-5 bg-muted/50 rounded-xl border border-border text-sm text-muted-foreground">
          <strong className="text-foreground">How this digest is maintained:</strong>{" "}
          This digest is reviewed and updated regularly to reflect the latest regulatory developments. All summaries are written in plain English for educational purposes. For legal advice or compliance decisions, always consult the original regulatory sources and qualified legal counsel.
        </div>
      </section>
    </PageLayout>
  );
